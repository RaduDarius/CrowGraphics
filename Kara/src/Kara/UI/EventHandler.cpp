...
namespace UI {
namespace {
// TODO: This should be removed because we will create our own Key ids.
ImGuiKey KeyToImGuiKey(int aKeyCode);
...
bool EventHandler::OnKeyPressedEvent(EventSystem::KeyPressedEvent &aEvent) {
  auto &io = ImGui::GetIO();
  auto key = KeyToImGuiKey(aEvent.GetKeyCode());
...
bool EventHandler::OnKeyTypedEvent(EventSystem::KeyTypedEvent &aEvent) {
  ...
  if (keyCode > 0 && keyCode < 0x10000) {
    io.AddInputCharacter(static_cast<char>(keyCode));
  }
  return false;
}
